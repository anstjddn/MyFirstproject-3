...
public class UnityCollision : MonoBehaviour
{
	 ...
	 * 물체가 복잡할경우 메시보다 여러가지 충돌체를 복합적으로 사용
	 * 충돌체가 충돌상황에 있을 경우 유니티 충돌 메시지를 받아 상황을 확인
	 ************************************************************************/

    // <유니티 충돌 메시지>
    private void OnCollisionEnter(Collision collision)        // 충돌체끼리 닿는 순간 호출
    {
        Debug.Log("OnCollisionEnter");
    }

    private void OnCollisionStay(Collision collision)       //계속 충돌되고 있을떄 호출
    {
        Debug.Log("OnCollisionStay");
    }

    private void OnCollisionExit(Collision collision)           // 충돌체끼지 떼어질떄 호출
    {
    ...
    // 트리거랑 같이 쓸려면 충돌체를 좀더 안쪽으로 잡고 트리거를 바깥영역으로 확장
    // <충돌체 종류>
